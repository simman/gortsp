...
var RTSPServerSampleCallback OnSampleCallback
...
func (impl *ServerHandleImpl) HandleAnnounce(svr *rtsp.RtspServer, req rtsp.RtspRequest, tracks map[string]*rtsp.RtspTrack) {
	...
	if vtrack, found := tracks["video"]; found {
		...
		vtrack.OnSample(func(sample rtsp.RtspSample) {
			...
			if RTSPServerSampleCallback != nil {
				RTSPServerSampleCallback(sample)
			}
...
func RunRtspServer(callback OnSampleCallback) {
	RTSPServerSampleCallback = callback
	addr := "0.0.0.0:18888"
